...
class TestWorkspaceRunnerGeneration(unittest.TestCase):
    ...
    def test_generated_runner_delegates_to_admin_module(self) -> None:
        """
        The generated runner should delegate to `python -m moovent_stack.admin`.
        
        Note: The actual port configuration (3000/4000/8000) is now handled by
        the admin module, not the generated script.
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            workspace._ensure_workspace_runner(root)
            content = (root / "run_local_stack.py").read_text(encoding="utf-8")
            # Should delegate to the admin module
            self.assertIn("moovent_stack.admin", content)
            self.assertIn("generated by moovent-stack", content)
